  ...
  run: async (client, message, args, prefix) => {
    ...
    if (!messageId) {
      return message.channel.send({
        embeds: [new EmbedBuilder().setDescription(`\`${prefix}gend <message id>\``)] 
      });
    }
...
async function end(member, messageId) {
  ...
  try {
    await giveaway.end();
    return {
      embeds: [new EmbedBuilder().setDescription(`${member.client.emoji.tick} ${member}: Ended the giveaway \`${messageId}\``)]
    };
  } catch (error) {
    console.log(error);
    return {
      embeds: [new EmbedBuilder().setDescription(`I was unable to end the giveaway \`${messageId}\``)]
    };
  }
}

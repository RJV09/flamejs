...
module.exports = {
  name: 'moderator',
  aliases: ['mod', 'mods', 'moderation', 'moderators'],
  category: 'reqrole',
  description: "Give Customroles to User",
  args: false,
  usage: 'moderator <user>',
  botPerms: [],
  userPerms: [],
  owner: false,
  ...
  run: async (client, message, args, prefix) => {
    ...
    if (!message.member.permissions.has('ManageRoles') && !message.member.roles.cache.has(reqrole)) {
      return message.channel.send({embeds: [ new EmbedBuilder().setDescription(`${client.emoji.cross} ${message.author}: You need the **required role** or **Manage Roles** permission to do so`)]})
    }
    ...
    let role_id = await client.db.get(`mod_${message.guild.id}`)
    ...
    if (!role_id || !role) {
      return message.channel.send({embeds: [new EmbedBuilder().setTitle(client.emoji.cross + ` Role Not Found!`).setDescription(`**Possible Reasons:**\n• Role Not Set\n• Role Deleted`)]})
    }

...
module.exports = {
  ...
  aliases: ['setuprole'],
  category: 'reqrole',
  description: 'Setup custom roles',
  args: true,
  ...
  run: async (client, message, args, prefix) => {
    ...
    if (type === 'config' || type === 'show') {
      ...
      let e = new EmbedBuilder().setDescription(
      ...
      let a = new EmbedBuilder()
        ...
        .setDescription(`
          **Required Role**: ${req ? `<@&${req}>` : "Role Isn't Set"}
    ...
    if (type === `partners` || type === `partner` || type === `prtner`) {
      ...
      return message.channel.send({embeds: [new EmbedBuilder().setDescription(`${client.emoji.tick} ${message.author}: Set the Partner role to <@&${role}>`)]});
    }

    if (type === `required` || type === `reqrole` || type === `requiredrole`) {
      client.db.set(`reqrole_${message.guild.id}`, roml.id);
      return message.channel.send({embeds: [new EmbedBuilder().setDescription(`${client.emoji.tick} ${message.author}: Set the Required role to <@&${role}>`)]});
    }

    if (type === `mod` || type === `moderator` || type === `moderation`) {
      client.db.set(`mod_${message.guild.id}`, roml.id);
      return message.channel.send({embeds: [new EmbedBuilder().setDescription(`${client.emoji.tick} ${message.author}: Set the Mod role to <@&${role}>`)]});
    }
    ...
    if (type === `bots` || type === `robot` || type === `bot`) {
      ...
      return message.channel.send({embeds: [new EmbedBuilder().setDescription(`${client.emoji.tick} ${message.author}: Set the Bots role to <@&${role}>`)]});
    }

    if (type === `staff` || type === `officials` || type === `official` || type === `worker`) {
      client.db.set(`staff_${message.guild.id}`, roml.id);
      return message.channel.send({embeds: [new EmbedBuilder().setDescription(`${client.emoji.tick} ${message.author}: Set the Staff role to <@&${role}>`)]});
    }

    if (type === `vip` || type === `premium` || type === `vvip`) {
      client.db.set(`vip_${message.guild.id}`, roml.id);
      return message.channel.send({embeds: [new EmbedBuilder().setDescription(`${client.emoji.tick} ${message.author}: Set the Vip role to <@&${role}>`)]});
    }

    if (type === `guest` || type === `guests`) {
      client.db.set(`guest_${message.guild.id}`, roml.id);
      return message.channel.send({embeds: [new EmbedBuilder().setDescription(`${client.emoji.tick} ${message.author}: Set the Guest role to <@&${role}>`)]});
    }

    if (type === `girl` || type === `qt` || type === `cutie` || type === `girls` || type === `cuties` || type === `ladki`) {
      client.db.set(`qt_${message.guild.id}`, roml.id);
      return message.channel.send({embeds: [new EmbedBuilder().setDescription(`${client.emoji.tick} ${message.author}: Set the Girl role to <@&${role}>`)]});
    }

    if (type === `artists` || type === `artist` || type === `kalakar`) {
      client.db.set(`artist_${message.guild.id}`, roml.id);
      return message.channel.send({embeds: [new EmbedBuilder().setDescription(`${client.emoji.tick} ${message.author}: Set the Artist role to <@&${role}>`)]});
    }

    if (type === `admins` || type === `administrator` || type === `admin`) {
      client.db.set(`admin_${message.guild.id}`, roml.id);
      return message.channel.send({embeds: [new EmbedBuilder().setDescription(`${client.emoji.tick} ${message.author}: Set the Admin role to <@&${role}>`)]});
    }

    if (type === `friend` || type === `friends` || type === `bhai` || type === `dost` || type === `specials`) {
      client.db.set(`friend_${message.guild.id}`, roml.id);
      return message.channel.send({embeds: [new EmbedBuilder().setDescription(`${client.emoji.tick} ${message.author}: Set the Friend role to <@&${role}>`)]});
    }

    if (type === `manager` || type === `management` || type === `managers`) {
      client.db.set(`manager_${message.guild.id}`, roml.id);
      return message.channel.send({embeds: [new EmbedBuilder().setDescription(`${client.emoji.tick} ${message.author}: Set the Manager role to <@&${role}>`)]});
    }

    if (type === `owner` || type === `own` || type === `king` || type === `owners`) {
      client.db.set(`owner_${message.guild.id}`, roml.id);
      return message.channel.send({embeds: [new EmbedBuilder().setDescription(`${client.emoji.tick} ${message.author}: Set the Owner role to <@&${role}>`)]});
    }

    if (type === `founder` || type === `founders` || type === `mainowner`) {
      client.db.set(`founder_${message.guild.id}`, roml.id);
      return message.channel.send({embeds: [new EmbedBuilder().setDescription(`${client.emoji.tick} ${message.author}: Set the Founder role to <@&${role}>`)]});
    }
  }
};

  ...
  run: async (client, message, args, prefix) => {
    ...
    if (!member.roles.cache.has(role.id)) {
      ...
      if (x) {
        return message.channel.send({
          embeds: [
            new EmbedBuilder()
              .setDescription(`${client.emoji.tick} ${message.author}: **Added** ${role} to ${member.user.tag}`)
          ],
        });
      }
      return;
    ...
    if (x) {
      return message.channel.send({
        embeds: [
          new EmbedBuilder()
            .setDescription(`${client.emoji.tick} ${message.author}: **Removed** ${role} from ${member.user.tag}`)
        ],
      });
    }
  }

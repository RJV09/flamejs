...
if(channel) {
    ...
    await message.channel.send({
        embeds:[new EmbedBuilder().setColor(client.color)            
            ...
            .addFields({ name: 'Types of Channel Logging Enabled', value: '\`Channel Create\`\n\`Channel Delete\`\n\`Channel update\`', inline: false })
            .addFields({ name: 'What happens now?', value: 'The bot will log member channel create, channel delete, channel update in the configured channel.', inline: false })],
    })

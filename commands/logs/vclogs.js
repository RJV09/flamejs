...
if(channel) {
    ...
    await message.channel.send({
        embeds:[new EmbedBuilder().setColor(client.color)            
            ...
            .addFields({ name: 'Types of Vc Logging Enabled', value: '\`Member Join\`\n\`Member Leave\`\n\`Member move\`', inline: false })
            .addFields({ name: 'What happens now?', value: 'The bot will log member vc join, leave, and move member in the configured channel.', inline: false })],

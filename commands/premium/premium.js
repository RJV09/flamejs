...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        const embed = new EmbedBuilder()
            ...
            .setFooter({ text: `Developed By FlaMe Team` });

            const row = new ActionRowBuilder().addComponents(
                new ButtonBuilder()
                    .setLabel('Premium')
                    .setStyle('Link')
                    .setURL('https://discord.gg/SCs6VBmspY')  // Removed trailing space here
            );
            

        let link = 'https://discord.gg/SCs6VBmspY ';

        if (!args[0]) {
            embed.setAuthor({
                name: `${client.user.username} Premium`,
                iconURL: client.user.displayAvatarURL(),
                url: 'https://discord.gg/SCs6VBmspY '
            })
            ...
            case 'revoke':
                ...
                if (!isp) {
                    return message.reply({
                        embeds: [
                            embed.setDescription(
                                `This server hasn't any type of premium subscription! If you are a subscriber, you can **upgrade** this server by typing \`${prefix}premium activate\`\nClick [here](${link}) to [Purchase](${link}).`
                            )
                        ],
                        components: [row]
                    });

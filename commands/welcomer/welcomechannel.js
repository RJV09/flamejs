            ...
            return message.channel.send({
                embeds: [
                    new EmbedBuilder()
                        .setColor(client.color)
                        .setDescription(
                            `You didn't mention a channel to set as the welcome channel.`
                        )
                ]
            });
        }
            ...
            return message.channel.send({
                embeds: [
                    new EmbedBuilder()
                        .setColor(client.color)
                        .setDescription(
                            `${client.emoji.cross} | The channel must be a text-based channel.`
                        )
                ]
            });
        }

        const response = await client.util.setChannel(settings, channel);
        return message.channel.send({
            embeds: [
                new EmbedBuilder()
                    .setColor(client.color)

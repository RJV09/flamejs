...
module.exports = class Util {
        ...
        try {
            ...
            if (funny === 'first') {
                _commands = this.client.commands
                    ...
                    .map((x) => `\`${x.name}\``)
                embed.addFields({
                    name: `**<:Satxler_antinuke:1181289584483643433> Antinuke \`[${_commands.length}]\`**`,
                    value: _commands.sort().join(', ')
                })
                interaction
                    .update({
                        embeds: [embed],
            ...
            if (funny === 'second') {
                _commands = this.client.commands
                    ...
                    .map((x) => `\`${x.name}\``)
                embed.addFields({
                    name: `**<:Satxler_moderator:1181290384576491561> Moderation \`[${_commands.length}]\`**`,
                    value: _commands.sort().join(', ')
                })
                interaction
                    .update({
            ...
            if (funny === 'third') {
                _commands = this.client.commands
                    ...
                    .map((x) => `\`${x.name}\``)
                embed.addFields({
                    name: `**<:Satxler_utility:1181291761667149886> Utility \`[${_commands.length}]\`**`,
                    value: _commands.sort().join(', ')
                })
                interaction
                    .update({
            ...
            if (funny === 'fourth') {
                _commands = this.client.commands
                    ...
                    .map((x) => `\`${x.name}\``)
                embed.addFields({
                    name: `**<:Satxler_autorole:1181290290238210158> Welcomer \`[${_commands.length}]\`**`,
                    value: _commands.sort().join(', ')
                })
                interaction
                    .update({
            ...
            if (funny === 'fifth') {
                _commands = this.client.commands
                    ...
                    .map((x) => `\`${x.name}\``)
                embed.addFields({
                    name: `**<:Satxler_mic:1181294198046072994> Voice \`[${_commands.length}]\`**`,
                    value: _commands.sort().join(', ')
                })
                interaction
                    .update({
            ...
            if (funny === 'six') {
                ...
                embed.addFields({
                    name: `**<:Customrole:1199024011045253140> Customrole \`[${cmd.length}]\`**`,
                    value: cmd.sort().join(', ')
                })
                await interaction
                    .update({
            ...
            if (funny === 'seven') {
                _commands = this.client.commands
                    ...
                    .map((x) => `\`${x.name}\``)
                embed.addFields({
                    name: `**<:logs:1200416495461732353> Logging \`[${_commands.length}]\`**`,
                    value: _commands.sort().join(', ')
                })
                interaction
                    .update({
            ...
            if (funny === 'eight') {
                ...
                embed.addFields({
                    name: `**<:Satxler_Automod:1205791245473943553> Automod \`[${cmd.length}]\`**`,
                    value: cmd.sort().join(', ')
                })
                await interaction
                    .update({
    ...
    async manageAfk(message, client) {
        try {
            // Step 1: Check for server-specific AFK status
            let data = await db.findOne({ Guild: message.guildId, Member: message.author.id });
    
            if (data) {
                // If the author is AFK in this server, remove their AFK status
                await data.deleteOne();
                return message.reply({
                    embeds: [
                        new EmbedBuilder()
                            .setColor(client.color)
                            .setDescription("I removed your AFK status in this server.")
                    ]
                });
            }
    
            // Step 2: Check for global AFK status
            data = await db.findOne({ Guild: 'global', Member: message.author.id });
            if (data) {
                // If the author is globally AFK, remove their global AFK status
                await data.deleteOne();
                return message.reply({
                    embeds: [
                        new EmbedBuilder()
                            .setColor(client.color)
                            .setDescription("I removed your global AFK status.")
                    ]
                });
            }
    
            // Step 3: If no AFK status is found, check the AFK status of the mentioned user
            const memberMentioned = message.mentions.users.first();
            if (memberMentioned) {
                // Check for server-specific AFK status of the mentioned user
                    ...
                    return message.reply({
                        embeds: [
                            new EmbedBuilder()
                                .setColor(client.color)
                                .setDescription(
                                    `<@${memberMentioned.id}> went AFK in this server <t:${Math.round(data.Time / 1000)}:R>\n\nReason: **${data.Reason}**`
                                )
                        ]
                    });
                }
    
                // Check for global AFK status of the mentioned user
                data = await db.findOne({ Guild: 'global', Member: memberMentioned.id });
                if (data) {
                    return message.reply({
                        embeds: [
                            new EmbedBuilder()
                                .setColor(client.color)
                                .setDescription(
                                    `<@${memberMentioned.id}> is globally AFK <t:${Math.round(data.Time / 1000)}:R>\n\nReason: **${data.Reason}**`
                                )
                        ]
                    });
                }
            ...
            // No AFK data found for the user or mentioned user
            return;
    ...
    async setGlobalAfk(message, reason, client) {
        try {
            // Step 1: Validate the reason for AFK
            if (!reason) {
                return message.reply({
                    embeds: [
                        new EmbedBuilder()
                            .setColor("Red")
                            .setDescription("You must provide a reason for going AFK.")
                    ]
                });
            }
    
            // Step 2: Check if the user already has a global AFK status
            const existingGlobalAfk = await db.findOne({ Guild: 'global', Member: message.author.id });
    
            if (existingGlobalAfk) {
                // Update the existing global AFK status
                existingGlobalAfk.Reason = reason;
                existingGlobalAfk.Time = Date.now();
                await existingGlobalAfk.save();
                return message.reply({
                    embeds: [
                        new EmbedBuilder()
                            .setColor(client.color)
                            .setDescription(`Your global AFK status has been updated.\nReason: **${reason}**`)
                    ]
                });
            }
    
            // Step 3: Create a new global AFK status if none exists
            const newGlobalAfk = new db({
                Guild: 'global',
            ...
            await newGlobalAfk.save();
            return message.reply({
                embeds: [
                    new EmbedBuilder()
                        .setColor(client.color)
                        .setDescription(`You are now globally AFK.\nReason: **${reason}**`)
                ]
            });
            ...
            message.reply({
                embeds: [
                    new EmbedBuilder()
                        .setColor("Red")
                        .setDescription("An error occurred while setting your global AFK status.")
                ]
            });
        }
    }
